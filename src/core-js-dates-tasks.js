...
function getTime(date) {
  ...
  const hour = format(newDate.getHours());
  const min = format(newDate.getMinutes());
  const sec = format(newDate.getSeconds());
  return `${hour}:${min}:${sec}`;
}
...
function getNextFriday(date) {
  const difMs = ((5 - date.getUTCDay() + 7) % 7 || 7) * 24 * 60 * 60 * 1000;
  const nextFriday = new Date();
  nextFriday.setTime(date.getTime() + difMs);
  return nextFriday;
}
...
function isDateInPeriod(date, period) {
  ...
  return (
    startDate.getTime() <= newDate.getTime() &&
    endDate.getTime() >= newDate.getTime()
  );
}
...
function getWeekNumberByDate(date) {
  ...
  const yearStart = new Date(Date.UTC(curDate.getUTCFullYear(), 0, 1));

  const weekNumber = Math.ceil(
    ((curDate - yearStart) / (1000 * 60 * 60 * 24) + 1) / 7
  );
...
function getNextFridayThe13th(date) {
  ...
  for (let i = 0; i < 12; i += 1) {
    if (currentDate > date && currentDate.getDay() === 5) {
      return currentDate;
    }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatToDate(currentDate));
